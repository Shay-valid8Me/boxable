...
public class DataTableTest {
  ...
  public void csvTestColWidths() throws IOException {
    ...
    String data =
        "Discription;narrow;this is wider;and this one is even wider\n"
  ...
  public void csvTestPortrait() throws IOException {
    String data =
        readData(
            "https://s3.amazonaws.com/misc.quodlibet.be/Boxable/Eurostat_Immigration_Applications.csv");
    // Initialize Document
    PDDocument doc = new PDDocument();
    PDPage page = new PDPage();
    doc.addPage(page);
  ...
  public void csvTestSimple() throws IOException {
    ...
    String data =
        readData(
            "https://s3.amazonaws.com/misc.quodlibet.be/Boxable/Eurostat_Energcy_Prices_Medium_Household.csv");
    // Initialize Document
    PDDocument doc = new PDDocument();
    PDPage page = new PDPage();
    // Create a landscape page
    ...
    Cell c2 =
        h2.createCell(
            100,
            "Eur per kWh for Medium Size Households.<br/>Source <i>http://ec.europa.eu/eurostat/tgm/table.do?tab=table&init=1&plugin=1&language=en&pcode=ten00117</i>");
  ...
  public void csvTestAdvanced() throws IOException {
    ...
    c1 =
        h1.createCell(
            100, "Same tabel with zebra design and first and last column with other collors");
    c1.setFillColor(new Color(144, 195, 212));
    dataTable.addHeaderRow(h1);
    t = new DataTable(dataTable, page);
    ...
    c1 =
        h1.createCell(
            100,
            "Same tabel with zebra design and first and last column with other collors with also zebra design");
    c1.setFillColor(new Color(144, 195, 212));
    dataTable.addHeaderRow(h1);
    t = new DataTable(dataTable, page);
    iterator = t.getDataCellTemplateEvenList().iterator();
    ...
    t =
        new DataTable(
            dataTable,
            page,
            new UpdateCellProperty() {

              @Override
              public void updateCellPropertiesAtColumn(Cell<PDPage> c, int column, int row) {
                if (column == 11) {
                  if (c.getText().startsWith("0,2")) c.setFillColor(c02);
                  if (c.getText().startsWith("0,1")) c.setFillColor(c01);
                }
              }
            });
    iterator = t.getDataCellTemplateEvenList().iterator();
