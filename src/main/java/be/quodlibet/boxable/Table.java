...
public abstract class Table<T extends PDPage> {
   ...
   * @param currentPage current page where {@link Table} will be drawn (some tables are big and can
   *     be through multiple pages)
  ...
  public void drawTitle(
    ...
    if (title == null) {
      ...
      if (drawDebug) {
        // margin
        PDStreamUtils.rect(
            tableContentStream, margin, yStart, width, headerBottomMargin, Color.CYAN);
      }
    }
  ...
  /**
   * Method to switch between the {@link PageProvider} and the abstract method {@link
   * Table#createPage()}, preferring the {@link PageProvider}.
   *
   * <p>Will be removed once {@link #createPage()} is removed.
   *
   * @return
   */
  ...
  protected T createPage() {
    throw new IllegalStateException(
        "You either have to provide a "
            + PageProvider.class.getCanonicalName()
  ...
  private void drawCellContent(Row<T> row, float rowHeight) throws IOException {
    ...
    for (Cell<T> cell : row.getCells()) {
      ...
      if (cell instanceof ImageCell) {
        ...
        if (imageCell.getUrl() != null) {
          ...
          PDRectangle position =
              new PDRectangle(
                  cursorX,
                  cursorY,
                  (float) (imageCell.getImage().getWidth()),
                  -(float) (imageCell.getImage().getHeight()));
        ...
        for (Map.Entry<Integer, List<Token>> entry :
            cell.getParagraph().getMapLineTokens().entrySet()) {

          // calculate the width of this line
          float freeSpaceWithinLine =
              cell.getParagraph().getMaxLineWidth()
      ...
      PDRectangle rectangle =
          new PDRectangle(cellStartX, yStart - cell.getHeight(), cell.getWidth(), cell.getHeight());
  ...
  private void drawVerticalLines(Row<T> row, float rowHeight) throws IOException {
    ...
    while (cellIterator.hasNext()) {
      ...
      float cellWidth = cellIterator.hasNext() ? cell.getWidth() : this.width - (xStart - margin);
  ...
  private void drawCellBorders(float rowHeight, Cell<T> cell, float xStart) throws IOException {
    ...
    if (bottomBorder != null) {
      ...
      drawLine(
          xStart,
          y,
          xStart + cell.getWidth() - (rightBorder == null ? 0 : rightBorder.getWidth()),
          y,
          bottomBorder);
    }
    ...
    if (leftBorder != null) {
      ...
      drawLine(
          x, yStart, x, yEnd + (bottomBorder == null ? 0 : bottomBorder.getWidth()), leftBorder);
    }
  }

  private void drawLine(float xStart, float yStart, float xEnd, float yEnd, LineStyle border)
      throws IOException {
  ...
  private void fillCellColor(
      Cell<T> cell, float yStart, float xStart, float rowHeight, float cellWidth)
      throws IOException {
   ...
   * /**
   *
   * @deprecated Use {@link #addHeaderRow(Row)} instead, as it supports multiple header rows
   ...
   *
   * <p>IMPORTANT: Doesn't acknowledge possible page break. Use with caution.
   *
  ...
  /**
   * Calculates minimum table height that needs to be drawn (all header rows + first data row
   * heights).
   *
   * @return height
   */
