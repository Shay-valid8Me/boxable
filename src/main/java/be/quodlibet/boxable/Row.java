...
public class Row<T extends PDPage> {
  ...
  public Cell<T> createImageCell(
      float width, Image img, HorizontalAlignment align, VerticalAlignment valign) {
   ...
   */
  public TableCell<T> createTableCell(
      float width,
      String tableData,
      PDDocument doc,
      PDPage page,
    ...
    TableCell<T> cell =
        new TableCell<T>(
            this, width, tableData, true, doc, page, yStart, pageTopMargin, pageBottomMargin);
    setBorders(cell, cells.isEmpty());
    cells.add(cell);
    return cell;
  }
   ...
   * @return New {@link Cell}
   */
  public Cell<T> createCell(
      float width, String value, HorizontalAlignment align, VerticalAlignment valign) {
  ...
  /**
   * Remove left border to avoid double borders from previous cell's right border. In most cases
   * left border will be removed.
   *
   * @param cell {@link Cell}
   * @param leftBorder boolean for drawing cell's left border. If {@code true} then the left cell's
