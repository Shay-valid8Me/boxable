...
public final class FontUtils {
  ...
  /**
   * {@link HashMap} for caching {@link FontMetrics} for designated {@link PDFont} because {@link
   * FontUtils#getHeight(PDFont, float)} is expensive to calculate and the results are only
   * approximate.
   */
  ...
  public static final PDType0Font loadFont(PDDocument document, String fontPath) {
    try {
      return PDType0Font.load(
          document, FontUtils.class.getClassLoader().getResourceAsStream(fontPath));
    } catch (IOException e) {
  ...
  /**
   * Retrieving {@link String} width depending on current font size. The width of the string in
   * 1/1000 units of text space.
   *
   ...
   * @throws IOException If reading the font file fails ======= >>>>>>> using FreeSans as default
   *     font and added new free fonts
   */
