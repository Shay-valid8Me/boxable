...
public class Cell<T extends PDPage> {
  ...
  private final List<CellContentDrawnListener<T>> contentDrawnListenerList =
      new ArrayList<CellContentDrawnListener<T>>();

  /**
   * Constructs a cell with the default alignment {@link VerticalAlignment#TOP} {@link
   ...
   * @param text
   * @param isCalculated
   * @see Cell#Cell(Row, float, String, boolean, HorizontalAlignment, VerticalAlignment)
   */
   ...
   * @param width absolute width in points or in % of table width (depending on the parameter {@code
   *     isCalculated})
   * @param text The text content of the cell
   * @param isCalculated If {@code true}, the width is interpreted in % to the table width
   ...
   *
   * <p>If cell has rotated value then {@link Paragraph} width is depending of {@link
   * Cell#getInnerHeight()} otherwise {@link Cell#getInnerWidth()}
   *
  ...
  /**
   * Gets the height of the single cell, opposed to {@link #getHeight()}, which returns the row's
   * height.
   *
   * <p>Depending of rotated/normal cell's value there is two cases for calculation:
   *
   * <ol>
   *   <li>Rotated value - cell's height is equal to overall text length in the cell with necessery
   *       paddings (top,bottom)
   *   <li>Normal value - cell's height is equal to {@link Paragraph}'s height with necessery
   *       paddings (top,bottom)
   * </ol>
   *
  ...
  public float getCellHeight() {
    ...
    if (isTextRotated()) {
      try {
        ...
        return getFont().getStringWidth(getText()) / 1000 * getFontSize()
            + getTopPadding()
   ...
   *
   * <p>If cell has rotated value then free vertical space is equal inner cell's height ({@link
   * #getInnerHeight()}) subtracted to the longest line of rotated {@link Paragraph} otherwise it's
   * just cell's inner height ( {@link #getInnerHeight()}) subtracted with width of the normal
   * {@link Paragraph}.
   *
   ...
   *
   * <p>If cell has rotated value then free horizontal space is equal cell's inner width ({@link
   * #getInnerWidth()}) subtracted to the {@link Paragraph}'s height otherwise it's just cell's
   * {@link #getInnerWidth()} subtracted with width of longest line in normal {@link Paragraph}.
   *
   * @return Free vertical space of the cell's.
   */
  ...
  /**
   * Sets the {@linkplain PDFont font} used for bold text, for example in {@linkplain
   * #isHeaderCell() header cells}.
   *
